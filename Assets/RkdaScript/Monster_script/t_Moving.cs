...
public class t_Moving : MonoBehaviour
{
    ...
    public Sprite[] tank_Sprite = new Sprite[7];
    ...
    public void Spawned(int tankcolor, Vector3 pos, GameObject target, GameManegiment manag)
    {
        this.target = target;
        this.manag = manag;
        tank_color = tankcolor + 1;
        hp = 300; // hp 초기화
        invi = false;
        gameObject.GetComponent<SpriteRenderer>().sprite = tank_Sprite[tankcolor];
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        ...
        if (tempbullet != null)
        {
            if (tempbullet.bulletcol == tank_color)
            {
        ...
        {
            if (templayzer.bulletcol == tank_color && !invi)
            {
                invi = true;
    ...
    void death()
    {
        effect.GetComponent<ParticleSystem>().startColor = new Color(colors[tank_color, 0], colors[tank_color, 1], colors[tank_color, 2]);
        Instantiate(effect, gameObject.transform.position, Quaternion.identity);
        manag.coin += Random.Range(5, 10);
        pool.push(gameObject);
    }
